...
const LogoGallery: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto py-24 px-6 relative">
      <div className="text-center mb-16 italic">
        ...
        <h2 className="text-3xl font-black uppercase tracking-tighter mt-2 text-white">Identity Matrix</h2>
      </div>

      {/* SEZIONE DESKTOP */}
      <div className="mb-20">
        <div className="flex items-center gap-3 mb-10 border-b border-white/5 pb-4 opacity-50">
          <Monitor size={18} className="text-copper" />
          <h2 className="text-sm font-bold uppercase tracking-widest">Desktop Core Assets</h2>
        </div>
      ...
      <div>
        <div className="flex items-center gap-3 mb-10 border-b border-white/5 pb-4 opacity-50">
          ...
          <h2 className="text-sm font-bold uppercase tracking-widest">Social Scaling Assets</h2>
        </div>
        ...
        {selectedId && selectedLogo && (
          ...
          >
            <motion.div 
              ...
              className={`bg-white/[0.03] border border-white/10 p-6 md:p-12 w-full relative flex flex-col items-center 
            ...
            >
              ...
              <div className="w-full">{selectedLogo.svg}</div>
              <div className="text-center mt-8 border-t border-white/10 pt-6 w-full">
...
const LogoCard = ({ logo, onClick, isSquare }: any) => (
  <motion.div 
    ...
    className={`cursor-zoom-in bg-white/[0.02] border border-white/5 p-4 relative group hover:bg-white/[0.05] transition-all flex flex-col justify-between ${isSquare ? 'aspect-square' : ''}`}
  >
    ...
    <div className="flex items-center justify-center h-full">{logo.svg}</div>
    {!isSquare && <div className="border-t border-copper/10 pt-2 mt-4 text-[9px] font-mono text-copper/60 uppercase">{logo.title}</div>}
  </motion.div>
);

/* --- SVG ASSETS (INTEGRALI) --- */

const AlphaSVG = () => (
  <svg viewBox="0 0 800 260" className="w-full h-auto">
    <path d="M 50,50 L 50,210 L 750,210" fill="none" stroke="#F5F5F7" strokeWidth="1" opacity="0.15"/>
    <g stroke="#F5F5F7" strokeWidth="0.8" fill="none" opacity="0.15">
    ...
    <text x="80" y="135" fill="#F5F5F7" style={{fontFamily:'sans-serif',fontWeight:900,fontSize:'54px',letterSpacing:'8px'}}>PR<tspan fill="#C47B5E">0</tspan>T<tspan fill="#C47B5E">0</tspan>M<tspan fill="#C47B5E">1</tspan>US</text>
    <text x="82" y="195" fill="#F5F5F7" opacity="0.45" style={{fontFamily:'monospace',fontSize:'12px',letterSpacing:'4px'}}>CORE.SYSTEM_V1.0 // PHOENIX_DATA</text>
  </svg>
);

const BetaSVG = () => (
  <svg viewBox="0 0 800 260" className="w-full h-auto">
    <text x="75" y="115" fill="#F5F5F7" style={{fontFamily:'sans-serif',fontWeight:700,fontSize:'44px',letterSpacing:'15px'}}>PR<tspan fill="#C47B5E">0</tspan>T<tspan fill="#C47B5E">0</tspan>M<tspan fill="#C47B5E">1</tspan>US</text>
...
const DeltaSVG = () => (
  <svg viewBox="0 0 800 260" className="w-full h-auto">
    ...
    <text x="75" y="115" fill="#F5F5F7" style={{fontFamily:'sans-serif',fontWeight:700,fontSize:'44px',letterSpacing:'15px'}}>PR0T0M1US</text>
...
const EpsilonSVG = () => (
  <svg viewBox="0 0 800 260" className="w-full h-auto">
    ...
    <circle className="star-blink" cx="630" cy="70" r="1.5" fill="#F5F5F7"/><circle className="star-blink" cx="680" cy="150" r="2" fill="#F5F5F7" style={{animationDelay:'0.5s'}}/>
    <circle className="star-blink" cx="610" cy="120" r="1.5" fill="#F5F5F7" style={{animationDelay:'1s'}}/><circle className="star-blink" cx="710" cy="95" r="2" fill="#F5F5F7" style={{animationDelay:'1.5s'}}/>
    <circle className="star-blink" cx="650" cy="180" r="1" fill="#F5F5F7" style={{animationDelay:'2s'}}/><circle cx="660" cy="110" r="3.5" fill="#C47B5E" opacity="0.75"/>
    <text x="75" y="115" fill="#F5F5F7" style={{fontFamily:'sans-serif',fontWeight:700,fontSize:'44px',letterSpacing:'15px'}}>PR<tspan fill="#C47B5E">0</tspan>T<tspan fill="#C47B5E">0</tspan>M<tspan fill="#C47B5E">1</tspan>US</text>
    <text x="75" y="160" fill="#F5F5F7" opacity="0.6" style={{fontFamily:'monospace',fontSize:'12px',letterSpacing:'3px'}}>CORE.SYSTEM_V1.0 // DESIGNED_FOR_ORDER</text>
    <g fill="#F5F5F7" opacity="0.6" textAnchor="end" style={{fontFamily:'monospace',fontSize:'11px'}}>
...
const SocialStaticSVG = () => (
  <svg viewBox="0 0 512 512" className="w-full h-full">
    <rect x="125" y="125" width="262" height="262" fill="none" stroke="#F5F5F7" strokeWidth="0.5" opacity="0.15"/>
    <rect x="110" y="110" width="292" height="292" fill="none" stroke="#C47B5E" strokeWidth="1" opacity="0.3"/>
    <line x1="110" y1="110" x2="140" y2="110" stroke="#F5F5F7" strokeWidth="3"/><line x1="402" y1="402" x2="372" y2="402" stroke="#F5F5F7" strokeWidth="3"/>
    <g fill="#F5F5F7" textAnchor="middle" style={{fontFamily:'sans-serif',fontWeight:800,fontSize:'36px',letterSpacing:'12px'}}>
